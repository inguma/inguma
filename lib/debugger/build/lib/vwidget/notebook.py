...
def appendToNotebook(notebook, view, totop=True):
    ...
    junk for free.
    """
    #label = gtk.Label(view.vwGetDisplayName())
    label = createTabLabel(view.vwGetDisplayName(), view, notebook)
...
def createTabLabel(title, page, notebook):
    box = gtk.HBox()
...
def createWindowForPage(page, x=300, y=400):
    # FIXME this should be a stand alone function
    # possibly in a notebook handling module
    win = gtk.Window()
    nb = prepNotebook()
