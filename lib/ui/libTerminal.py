...
#    #set the tab properties
#    self.set_property('homogeneous', True)
#    #we do not show the tab if there is only one tab in total
##    self.set_property('show-tabs', False) 
#
#  def new_tab(self, command=''):
#    #we create a image to put in the tab
...
#    #we pass the child of the tab so we can find the
#    #tab back upon closure
#    self.label = self.create_tab_label(command, self.image)
#    self.label.show_all()
#    
#    self.set_tab_label_packing(self.image, True, True, gtk.PACK_START)
#    self.set_tab_label(self.image, self.label)
#    self.image.show_all()
#    self.set_current_page(nbpages)
#
#    term = vte.Terminal()
...
#    box = gtk.HBox()
#    icon = gtk.Image()
#    icon.set_from_stock(gtk.STOCK_NEW, gtk.ICON_SIZE_MENU)
#    label = gtk.Label('Bash')
#    closebtn = gtk.Button()
#    #the close button is made of an empty button
#    #where we set an image
