...
#import lib.ui.bokken.statusbar as statusbar
...
class MainApp:
    ...
    def __init__(self, target, backend, inguma):
...
#        self.target = dialog.file
        ...
        if self.backend == 'pyew':
            import lib.ui.bokken.core as core
            self.uicore = core.Core('low', False, None)
...
#            self.uicore = radare_core.Core(dialog.radare_lower, dialog.analyze_bin, dialog.asm_syn ,dialog.use_va, dialog.asm_byt, dialog.progress_bar)
        ...
        if self.backend == 'pyew':
            import lib.ui.bokken.pyew_toolbar as toolbar
            self.topbuttons = toolbar.TopButtons(self.uicore, self)
        elif self.backend == 'radare':
            import lib.ui.bokken.radare_toolbar as toolbar
            self.topbuttons = toolbar.TopButtons(self.uicore, self)
...
#        self.inguma.bokken_tb.menu.create_view_menu()
...
#                self.inguma.bokken_tb.diff_tb.set_sensitive(False)
...
#        Gtk.main()
    ...
    def show_empty_gui(self):
        self.inguma.bokken_tb.throbber.running('start')
    ...
    def show_file_data(self):
        ...
        if 'radare' in self.uicore.backend and platform.system() != 'Windows':
            ...
            self.inguma.bokken_tb.throbber.running('')
        ...
        self.inguma.bokken_statusbar.add_text(info, VERSION)
        self.inguma.bokken_statusbar.hide_all()
        self.inguma.bokken_statusbar._statusbar.show_all()
    ...
    def merge_dasm_rightextview(self):
        ...
        if 'radare' in self.uicore.backend:
            ...
            self.tviews.search(self, link_name)
            self.tviews.right_notebook.finish_dasm()
            self.inguma.bokken_tb.menu._finish_dasm()
        elif 'pyew' in self.uicore.backend:
            if self.uicore.core.format in ['PE', 'ELF']:
                if self.uicore.core.ep:
                    link_name = "0x%08x" % self.uicore.core.ep
    ...
    def disable_all(self):
        #self.inguma.bokken_tb.disable_all()
        self.tviews.set_sensitive(False)

    def enable_all(self):
        self.inguma.bokken_tb.set_sensitive(True)
        self.inguma.bokken_tb.enable_all()
    ...
    def load_new_file(self, dialog, target):
        ...
        if self.target:
            ...
            import lib.ui.bokken.core_functions as core_functions
            core_functions.repaint()

        # Clean UI
        self.inguma.bokken_tb.menu.delete_view_menu()
        self.tviews.left_buttons.remove_all()
        self.tviews.right_notebook.remove_tabs()
        self.tviews.left_treeview.remove_columns()
        self.inguma.bokken_statusbar.remove_all()
        ...
        # Show UI
        self.enable_all()
        self.inguma.bokken_statusbar.show_all()
